...
def roi_appearance(
    ...
    output = dense_features.new_zeros(batch, time, num_objects, channels)
    ...
    feature_map = dense_features.permute(0, 1, 4, 2, 3)
    feature_map = feature_map.reshape(batch * time, channels, grid_h, grid_w)
    ...
    rois = torch.cat((image_index[:, None], selected_boxes), dim=1)
...
class CoarseModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.global_projection = nn.Sequential(nn.Linear(768, 384), nn.LayerNorm(384))
        ...
        self.entry_head = nn.Sequential(nn.Linear(384, 128), nn.GELU(), nn.Linear(128, 2))
        self.collision_head = nn.Sequential(nn.Linear(384, 128), nn.GELU(), nn.Linear(128, 2))

        self.video_pool = VideoPool()
        self.direction_head = nn.Sequential(
            nn.Linear(384, 128), nn.GELU(), nn.Dropout(0.1), nn.Linear(128, 2)
        )
        self.evasion_head = nn.Sequential(
            nn.Linear(384, 128), nn.GELU(), nn.Dropout(0.1), nn.Linear(128, 1)
        )
    ...
    def forward(
        ...
        if tubelets != 16 or tuple(tubelet_geometry.shape[2:]) != expected_geometry_shape:
        ...
        roi_features = roi_appearance(dense_features, boxes_in_grid, object_valid)
        roi_token = self.roi_projection(roi_features)
        geometry_token = self.geometry_embedding(tubelet_geometry)
        object_tokens = torch.cat((roi_token, geometry_token), dim=-1)
        ...
        spatial_tokens = torch.cat((global_token[:, :, None], object_tokens), dim=2)
        spatial_tokens = spatial_tokens.reshape(batch * tubelets, 13, 384)
        global_valid = torch.ones(batch, tubelets, 1, dtype=torch.bool, device=dense_features.device)
        spatial_valid = torch.cat((global_valid, object_valid), dim=2)
        spatial_valid = spatial_valid.reshape(batch * tubelets, 13)

        interaction = self.spatial_transformer(spatial_tokens, spatial_valid)[:, 0]
        interaction = interaction.reshape(batch, tubelets, 384)

        tubelet_valid = bin_valid.reshape(batch, 16, 2).any(dim=-1)
        temporal_hidden = self.temporal_transformer(
            interaction + sinusoidal(tubelet_valid), tubelet_valid
        )

        entry_logits = self.entry_head(temporal_hidden).reshape(batch, 32)
        collision_logits = self.collision_head(temporal_hidden).reshape(batch, 32)
        entry_logits = entry_logits.masked_fill(~bin_valid, -torch.inf)
        collision_logits = collision_logits.masked_fill(~bin_valid, -torch.inf)

        video_embedding = self.video_pool(temporal_hidden, tubelet_valid)
        return {
...
class FineModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.roi_projection = nn.Sequential(nn.Linear(384, 256), nn.LayerNorm(256))
        self.geometry_embedding = GeometryMLP()
        self.spatial_transformer = Transformer(num_layers=2)
        ...
        self.boundary_projection = nn.Linear(1152, 384)
        self.boundary_norm = nn.LayerNorm(384)

        self.entry_head = nn.Sequential(nn.Linear(384, 128), nn.GELU(), nn.Linear(128, 1))
        self.collision_head = nn.Sequential(nn.Linear(384, 128), nn.GELU(), nn.Linear(128, 1))
        self.entry_state_head = nn.Linear(384, 1)
        self.collision_state_head = nn.Linear(384, 1)
    ...
    def forward(
        ...
        object_tokens = torch.cat(
            (self.roi_projection(roi_features), self.geometry_embedding(geometry)), dim=-1
        )
        object_tokens = object_tokens * object_valid[..., None]

        spatial_tokens = torch.cat((global_tokens[:, :, None], object_tokens), dim=2)
        spatial_tokens = spatial_tokens.reshape(batch * frames, 13, 384)
        spatial_valid = torch.cat((time_valid[:, :, None], object_valid), dim=2)
        spatial_valid = spatial_valid.reshape(batch * frames, 13)

        frame_states = self.spatial_transformer(spatial_tokens, spatial_valid)[:, 0]
        frame_states = frame_states.reshape(batch, frames, 384) * time_valid[..., None]

        temporal_states = self.conv_kernel_3(frame_states, time_valid)
        temporal_states = self.conv_kernel_5(temporal_states, time_valid)
        temporal_states = self.temporal_transformer(
            temporal_states + sinusoidal(time_valid), time_valid
        )

        # The first frame uses itself as the previous state by construction.
        previous_frame = torch.cat((frame_states[:, :1], frame_states[:, :-1]), dim=1)
        boundary_input = torch.cat(
            (previous_frame, frame_states, frame_states - previous_frame), dim=-1
        )
        ...
        padded = ~time_valid
        return {
            "entry_logits": self.entry_head(final_states).squeeze(-1).masked_fill(padded, -torch.inf),
            "collision_logits": self.collision_head(final_states).squeeze(-1).masked_fill(padded, -torch.inf),
            "entry_state_logits": self.entry_state_head(final_states).squeeze(-1),
            "collision_state_logits": self.collision_state_head(final_states).squeeze(-1),
        }
