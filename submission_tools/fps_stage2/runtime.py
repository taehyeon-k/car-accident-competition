"""FPS-independent geometry-adapted DINOv3-S temporal probe.
...
def predict(data_dir, model_dir):
    ...
    if state.get('sampling_version') != 'normalized-clip-v1' or state.get('sample_count') != config['sample_count']:
        raise ValueError('Stage 2 checkpoint and normalized sampling configuration do not match')
    ...
    for folder in folders:
        ...
        positions = normalized_indices([i for i, _ in pairs], int(config['sample_count']))
        selected = [pairs[i] for i in positions]
        unique_positions, inverse = np.unique(positions, return_inverse=True)
        unique_pairs = [pairs[i] for i in unique_positions]
        unique_features = encode(backbone, unique_pairs, device, int(config['batch_size']), int(config['workers']))
        features = unique_features[:, torch.as_tensor(inverse, device=device)]
