...
from stage3.trainer.metrics import TrainingCompetitionMetrics
...
def run():
    ...
    train_actual, train_expected = TrainingCompetitionMetrics(), TrainingCompetitionMetrics()
    for batch in trainer.val_loader:
        output = trainer.ema.model(batch['motion'], batch['physics'], batch['lengths'])
        train_actual.update(output, batch, CFG, accelerator)
    for batch in loader:
        output = reference.ema.model(batch['motion'], batch['physics'], batch['lengths'])
        train_expected.update(output, batch, CFG)
    assert train_actual.compute() == train_expected.compute()
    for key, value in train_actual.compute().items():
        assert abs(value - actual[key]) < 1e-12, (key, value, actual[key])
