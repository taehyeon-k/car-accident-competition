...
# --------------------------------------------------------------------------- #
# Stage 2 / Stage 3 placeholders (unchanged)
# --------------------------------------------------------------------------- #
def predict_stage2(data_dir, model_dir):
    del model_dir
    data_path = _resolve_stage_dir(Path(data_dir), "stage2")
    rows = []
    for item_id, item_path in _discover_stage2_items(data_path):
...
def predict_stage3(data_dir, model_dir):
    del model_dir
    data_path = _resolve_stage_dir(Path(data_dir), "stage3")
    rows = []
    for video_id, video_path in _discover_video_items(data_path):
        sample_count = _video_frame_count(video_path)
        rows.extend(
            {
                "ID": video_id,
